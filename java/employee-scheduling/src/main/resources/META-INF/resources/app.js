...
const UNAVAILABLE_COLOR = '#8f8f8f' // Tango Scarlet Red
    ...
    schedule.resources.sort((a, b) => a.team.localeCompare(b.team)).forEach((resources, index) => {
        ...
        resources.unavailableDates.forEach((rawDate, dateIndex) => {
            ...
            const byResourceDemandElement = $(`<div/>`)
                .append($(`<h5 class="card-title mb-1"/>`).text("Absent"));
        ...
        resources.undesiredDates.forEach((rawDate, dateIndex) => {
            ...
            const end = date.plusDays(1).atStartOfDay().toString();
            const byResourceDemandElement = $(`<div/>`)
                .append($(`<h5 class="card-title mb-1"/>`).text("Private"));
            byResourceItemDataSet.add({
        ...
        if (demand.resource == null) {
            ...
            const byConstructionSiteDemandElement = $('<div class="card-body p-2"/>')
                .append($(`<h5 class="card-title mb-2"/>)`)
                    .append("Unassigned"))
                .append($('<div/>')
                    .append($(`<span class="badge me-1 mt-1" style="background-color:#d3d7cf">${demand.requiredResourceCategory}</span>`)));
            ...
            const skillColor = demand.requiredResourceCategory === demand.resource.resourceCategory ? '#ef2929' : '#8ae234';
            const byResourceDemandElement = $('<div class="card-body p-2"/>')
                .append($(`<h5 class="card-title mb-2"/>)`)
                    .append(demand.constructionSite))
                .append($('<div/>')
                    .append($(`<span class="badge me-1 mt-1" style="background-color:${skillColor}">${demand.requiredResourceCategory}</span>`)));
            const byConstructionSiteDemandElement = $('<div class="card-body p-2"/>')
                .append($(`<h5 class="card-title mb-2"/>)`)
